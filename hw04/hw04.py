...
def balanced(m):
    ...
    if left_mass * left_length == right_mass * right_length and balanced(end(left(m))) and balanced(end(right(m))):
        #还要保证手里拿的还是平衡的
        return True
    else:
        return False
    ...
    if is_leaf(t):
        ...
        for i in branches(t):
            ...
            if now > max:
                max = now
        return label(t) + max
    #return label(t) + max([max_path_sum(b) for b in branches(t)])#还可以这样
