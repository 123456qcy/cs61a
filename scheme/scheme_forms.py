...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        rest = scheme_eval(expressions.rest.first, env)#注意是.rest.first，若为.rest，则
        #(define size 2) 传入的为 Pair(2, nil),在scheme_eval中被当做Pair表达式对待，即认为
        #‘2’是符号，返回 2 not callable 的错误
        env.define(signature, rest)
        return signature
        # END PROBLEM 4
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    return expressions.first#大概是所有的expressions都是pair结构，且以nil结尾罢
    # END PROBLEM 5
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            # END PROBLEM 13
        expressions = expressions.rest#官方用法吗，哈哈
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        formal = bindings.first#let内的第一个括号
        validate_form(formal, 2, 2)#每个括号内包含一个name和一个val
